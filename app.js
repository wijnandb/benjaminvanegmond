...
(function () {
  ...
  // ---- Fetch upcoming matches (static JSON → TheSportsDB → fallback) ----
  async function fetchNextMatch() {
    // 1. Try static JSON generated by GitHub Actions (Football-Data.org)
    try {
      const res = await fetch('data/matches.json');
      if (res.ok) {
        const data = await res.json();
        if (data.nextMatches && data.nextMatches.length > 0) {
          applyStaticMatchData(data.nextMatches[0]);
          return;
        }
      }
    } catch (e) {
  ...
  function applyStaticMatchData(match) {
    const isHome = match.homeTeam.id === 678;
    const opponent = isHome ? match.awayTeam : match.homeTeam;
    const matchDate = new Date(match.utcDate);
    const venue = match.venue || 'Johan Cruijff ArenA';
    const round = match.matchday ? 'Matchday ' + match.matchday : '';
    ...
    document.getElementById('opponentName').textContent = opponent.name.toUpperCase();

    if (opponent.crest) {
      const crestEl = document.getElementById('opponentCrest');
      crestEl.innerHTML = `<img src="${opponent.crest}" alt="${opponent.name}" style="width:60px;height:60px;object-fit:contain;">`;
    }
  ...
  function applyTsdbMatchData(event) {
